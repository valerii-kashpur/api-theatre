...
class ReservationViewSetUnauthenticatedTests(APITestCase):
    ...
    def setUp(self):
        ...
        Ticket.objects.create(
            row=1, seat=1, performance=performance, reservation=reservation
        )

    def test_reservation_list_unauthenticated(self):
        """Test that an unauthenticated user
         cannot list reservations (returns 401 Unauthorized)."""
    ...
    def test_reservation_create_unauthenticated(self):
        ...
        data = {
            "tickets": [{"row": 2, "seat": 2, "performance": performance.id}]
        }
...
class ReservationViewSetAuthenticatedUserTests(APITestCase):
    ...
    def test_reservation_list_authenticated(self):
        """Test that an authenticated non-admin
         user can list their reservations."""
        url = "/api/theatre/reservations/"
        response = self.client.get(url)
    ...
    def test_reservation_create_authenticated(self):
        ...
        reservation = (
            Reservation.objects.filter(user=self.user)
            .order_by("-created_at")
            .first()
        )
        self.assertIsNotNone(reservation)
        self.assertEqual(
            Ticket.objects.filter(reservation=reservation).count(), 1
        )

    def test_reservation_pay_authenticated(self):
        """Test that an authenticated non-admin
         user can initiate a payment for a reservation."""
    ...
    def test_reservation_pay_authenticated_already_processed(self):
        """Test that an authenticated non-admin
         user cannot pay for an already processed reservation."""
        ...
        self.assertEqual(
            response.data["error"], "Reservation already processed"
        )

    def test_reservation_pay_authenticated_invalid_payment_method(self):
        """Test that an authenticated non-admin
         user gets an error with an invalid payment method."""
        url = f"/api/theatre/reservations/{self.reservation.id}/pay/"
        data = {"payment_method_id": "invalid_pm_id"}
        response = self.client.post(url, data, format="json")
    ...
    def test_reservation_confirm_payment_authenticated(self):
        """Test that an authenticated non-admin
         user can confirm a payment for a reservation."""
        pay_url = f"/api/theatre/reservations/{self.reservation.id}/pay/"
        pay_data = {"payment_method_id": "pm_card_visa"}
        ...
        confirm_url = (
            f"/api/theatre/reservations/{self.reservation.id}/confirm-payment/"
        )
    ...
    def test_reservation_confirm_payment_authenticated_missing_payment_intent(
        ...
        """Test that an authenticated non-admin
         user gets an error when confirming without payment intent ID."""
        url = (
            f"/api/theatre/reservations/{self.reservation.id}/confirm-payment/"
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["error"], "Payment intent ID is required"
        )
    ...
    def test_reservation_confirm_payment_authenticated_invalid_payment_intent(
        ...
        """Test that an authenticated non-admin
         user gets an error with an invalid payment intent ID."""
        url = (
            f"/api/theatre/reservations/{self.reservation.id}/confirm-payment/"
        )
        data = {"payment_intent_id": "invalid_pi_id"}
        response = self.client.post(url, data, format="json")
...
class ReservationViewSetAuthenticatedAdminTests(APITestCase):
    ...
    def test_reservation_list_authenticated(self):
        ...
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(
            response.data["results"][0]["tickets"][0]["row"], 1
        )
        self.assertEqual(
            response.data["results"][0]["tickets"][0]["seat"], 1
        )
